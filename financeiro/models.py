...
class Validacao(models.Model):
    ...
    def to_JSON(self):
        ...
        #return json.dumps({'TIPO':self.tipo,'IDOBJ':self.idObj,'VALIDACAO':self.validacao,'COMENTARIO':self.comentario}, default = lambda o: o.__dict__, sort_keys = True, indent = 4)
        return json.dumps({'TIPO':self.tipo,'IDOBJ':self.idObj,'VALIDACAO':self.validacao,'COMENTARIO':self.comentario})
    def object_decoder(obj):
        if '__type__' in obj and obj['__type__'] == 'Validacao':
            return User(obj['tipo'], obj['idObj'], obj['validacao'], obj['comentario'])
        return obj
